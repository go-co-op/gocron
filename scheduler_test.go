...
func TestScheduler_CalculateNextRun(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "every last day of the month started on january 1 in leap year should run on february 29", job: &Job{interval: 1, unit: months, daysOfTheMonth: []int{-1}, lastRun: januaryFirst2020At(0, 0, 0)}, wantTimeUntilNextRun: 30*day + 29*day},
		{name: "every last day of the month started on january 1 in non-leap year should run on february 28", job: &Job{interval: 1, unit: months, daysOfTheMonth: []int{-1}, lastRun: januaryFirst2019At(0, 0, 0)}, wantTimeUntilNextRun: 30*day + 28*day},
